...
export const SwipeCard = ({ scenario, onDecision }: SwipeCardProps) => {
  ...
  return (
    <div 
      className="relative w-full max-w-[340px] h-full flex flex-col items-center justify-center cursor-grab active:cursor-grabbing px-2 py-1"
      ...
      >
        <SlantedContainer className={cn(
          ...
          <div className="space-y-3 pt-2">
            <p className="text-[14px] leading-relaxed font-headline font-bold text-white tracking-tight">
              {scenario.scenario}
            </p>
          </div>
          ...
          <div className="mt-8 flex-1 flex flex-col justify-end">
            <div className="grid grid-cols-2 gap-3 h-28 relative">
              <div 
                className={cn(
                  ...
                  isLeft && dragX < -15 ? "bg-destructive border-white/40 scale-105 z-10" : "bg-white/5 border-transparent"
                )}
              ...
              >
                ...
                <div className="text-[10px] font-headline font-bold text-white leading-tight">
                  {scenario.leftOption}
                </div>
              </div>
              ...
              <div 
                className={cn(
                  "flex flex-col gap-1.5 p-3 rounded-lg border text-right transition-all duration-200",
                  isRight && dragX > 15 ? "bg-primary border-white/40 scale-105 z-10" : "bg-white/5 border-transparent"
                )}
            ...
            {Math.abs(dragX) < 15 && (
              <div className="text-center mt-6 flex items-center justify-center gap-3 animate-pulse opacity-40">
                <ChevronLeft className="w-4 h-4 text-destructive" />
                <span className="text-[9px] font-headline uppercase tracking-[0.4em] font-black">Swipe Action</span>
